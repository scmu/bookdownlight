...
updateThmCount t ch = do
  count <- get
  let (seccnt, [th, lm, df, ep, ec], contcnt) = count
  let (counter, ids) = case t of
                        1 -> ([th+1, lm, df, ep, ec], [ch, th+1]) -- theorem
                        2 -> ([th, lm+1, df, ep, ec], [ch, lm+1]) -- lemma
                        3 -> ([th, lm, df+1, ep, ec], [ch, df+1]) -- definition
                        4 -> ([th, lm, df, ep+1, ec], [ch, ep+1]) -- example
                        5 -> ([th, lm, df, ep, ec+1], [ch, ec+1]) -- exercise
...
updateContCount t ch = do
  count <- get
  let (seccnt, thmcnt, [fn, attr]) = count
  let (counter, ids) = case t of
                  1 -> ([fn+1, attr], [ch, fn+1])
                  2 -> ([fn, attr+1], [ch, attr+1])
