...
entry =
   do entryType <- char '@' >> identifier
      braces $
        do idn <- Parsec.try bibIdentifier
           attrs <- (comma >> sepEndBy assignment comma)
           return (Entry.Cons entryType idn attrs
                      (parseAuthors' (lookup "author" attrs)))
...
parseAuthors' :: Maybe String -> [Entry.AuthorName]
parseAuthors' = maybe [] parseAuthors

parseAuthors :: String -> [Entry.AuthorName]
parseAuthors allauthors =
    map parseAuthor . splitAuthorList .
      filter (not . flip elem ("\n\t" :: String))
       $ allauthors
 where parseAuthor name =
        let (surname, firstName) = break (','==) name
        in (surname, dropWhile (flip elem (", " :: String)) firstName)
